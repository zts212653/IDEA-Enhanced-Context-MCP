...
> **当前阶段提示**：当 PSI 缓存文件不存在时，Bridge 会回退到 regex index，这在 `/api/info` / MCP `health_check` 里会以 `dataSource: "regex"` 暴露。插件上传时会默认按 `projectName` 写成 `.idea-bridge/psi-cache-<project>.json`（可用 `BRIDGE_PSI_CACHE`/`BRIDGE_PSI_CACHE_DIR` 指定），建议确认 `/api/info` 中的 `psiCachePath` 指向期望的缓存。
